...
void dp(int* dfn, int* f) {
   ...
   for (int i = 1; i <= idx; i++) {
      ...
      // 0-1 knapsack: to pick child node must pick one item from the current node
      for (int j = m; j >= 0; j--) {
         if (j >= 1) {
            f[i * (m + 1) + j] = max(f[i * (m + 1) + j], f[(i - 1) * (m + 1) + (j - 1)] + va[node]);
         }
         f[i * (m + 1) + j] = max(f[i * (m + 1) + j], f[(i - sz[node]) * (m + 1) + j]);
      }
      ...
      // multiple knapsack on the current node
      for (int j = 0; j <= m; j++) {
         int cur_val = f[i * (m + 1) + j] - j * va[node];
         ...
         f[i * (m + 1) + j] = max(f[i * (m + 1) + j], q2[head] + j * va[node]);
      }
      cnt[node]++;
   }
}
